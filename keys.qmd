...
SSH keys are required to access the UCHC HPC system through an SSH connection. This page will walk you through the process of creating and using SSH keys to access the UCHC HPC system.
...
To be able to login using SSH keys, you will need to:

1. Create an SSH key pair on your local computer. 
2. Copy the public key to the UCHC HPC system. 

Only ed25519 keys are accepted.

### 1. Create SSH keys
You can create a public and private SSH key pair using your SSH client in `Windows PowerShell` or a MacOS / Linux `Terminal`. If using Windows Subsystem for Linux (WSL), you can use the Linux `Terminal` instructions.  
...
If you see files named `id_ed25519.pub` and `id_ed25519`, you already have an SSH key pair and can skip ahead to step the [Copy key to UCHC HPC system](#copy-key) section. If you do not see these files or get an error message, you will need to create a new SSH key pair.
...
### 2. Copy Public SSH Key to the UCHC HPC system {#copy-key}
...
::: {.panel-tabset}
#### MacOS / Linux Terminal
On MacOS or Linux operating systems, you can easily copy the public key to your clipboard by running the following command in the terminal application: 
```bash
pbcopy < ~/.ssh/id_ed25519.pub
```

#### Windows PowerShell
On Windows operating systems, you can easily copy the public key to your clipboard by running the following command in the PowerShell application: 
```bash
