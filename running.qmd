...
### Example
Below is an example which also demonstrates the use of [SLURM environment variables](https://slurm.schedmd.com/sbatch.html#SECTION_INPUT-ENVIRONMENT-VARIABLES) which access the values of the `--job-name` and `--cpus-per-task` arguments in the job script. Note the use of the `hostname` command to show the node that the job is running on. This information can be useful for debugging jobs. Particularly if you request help from the CBC as we will ask for this information.
...
#### \-\-qos
The qos (quality of service) argument constrains or modifies characteristics of a job. A qos argument must be specified for every job run on the Mantis cluster. This argument will usually be the same as that given for the partition if specifying a partition. If using the `general` paritition, only the qos needs to be specified.  
...
#### \-\-export
This specifies which environment variables to export to the job. The default on the UConn Health HPC system is `NONE` which means that no environment variables are exported. If you want to export all environment variables, use `ALL`. You can also specify a comma separated list of specific environment variables to export. For example, `--export=VAR1,VAR2` would export the environment variables VAR1 and VAR2 to the job.
...
### himem
This parition contains nodes suitable for jobs that have very large memory requirements. These nodes have 128 AMD CPUs and 1-2 TB of ram. Each node in this partition has one NVIDIA A100 GPU. Jobs cannot run in this partition unless &ge;500 GB of memory is requested. Only two jobs per user will run concurrently in this partition.
