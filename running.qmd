...
# Premtible QOS
The preemptible qos is available on the Mantis cluster. Jobs running with this qos may be preempted by higher priority jobs. This qos is intended for users who are willing to have their jobs interrupted in exchange for potentially lower wait times when nodes are not in use by higher priority users. Jobs with short run times or jobs that can be restarted are good candidates for this qos. This qos can be used with any of the Mantis partitions.
