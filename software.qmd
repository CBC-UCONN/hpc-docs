...
```bash
srun --qos=interactive --mem=8gb --pty bash
```
...
::: {.callout-important}
### Environment Modules
The integrated terminal in VSCode running with VSCode Tunnelswill not initialize environment modules by default. 

This can be resolve by running `source /etc/profile.d/modules.sh` in the integrated terminal or adding the command to your `~/.bashrc` file.

Alternatively, you can configure VSCode to ensure that a login shell is used for the integrated terminal, which will automatically load environment modules. This can be done by adding the following to your settings.json file: 
```json
{
