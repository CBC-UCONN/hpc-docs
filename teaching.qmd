...
Course user IDs will be `mcb5672usr1`, `mcb5672usr2` etc. The course instructor will be provided with a table listing user IDs and passwords (which can be changed **HOW DO YOU CHANGE PASSWORDS ON STUDENT ACCOUNTS????**) for each account. 
...
With updated security requirements, course account access to Mantis is only through a special instance of Open OnDemand and requires 2-factor authentication with an authenticator app **NEED A CLEAR EXPLANATION HERE**. Several apps are suitable, including Google Authenticator, Microsoft Authenticator and FreeOTP. Once configured these apps generate [time-based one-time passwords](https://en.wikipedia.org/wiki/Time-based_one-time_password)(TOTP) as a second form of authentication in addition to your user ID and password. 
...
Follow the instructions, retrieve the TOTP code from the OTP application you have chosen, enter the code in the One-time code field and click “Submit”. You will be logged into the Mantis Open OnDemand portal. 


![](img/authentication2.jpg)
...
**TENTATIVE PLEASE PROVIDE FEEDBACK**
...
If users wish to download files from their home directories select the `Files` menu and then `Home Directory`. Files can be downloaded from here. 

Upload is not possible (**I THINK??**) via Open OnDemand, but students can use [Globus](transfer.qmd#globus). It is not recommended for anyone with a regular CAM HPC account to try to link a student account. 