...
### Wildcards
Secure copy also supports the use of wildcards such as `*` for the remote path as long as wildcard characters are escaped with a `\`.  For example, to copy all `.txt` files from your UConn Health HPC file system home directory to your local computer:
...
There is an additional host that can be used to transfer data without an SSH key. This host is only accessible through the CAM VPN or the UConnHealth-SECURE WiFi or wired ethernet. This host requires your CAM password for authentication if you have not setup an SSH key.
...
::: {.panel-tabset}
## Windows 11
1. Connect to the [CAM VPN](connect.qmd#cam-vpn) if not on the UCHC secure WiFi network.
